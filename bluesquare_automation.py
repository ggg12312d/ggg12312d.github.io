...
def generate_blog_post(info):
    ...
    ### 🅿️ 성수동 주차 걱정? 지니가 해결해드릴게요!
    성수동 나들이 갈 때 차 가져갈지 항상 고민이죠. 저도 매번 주차 때문에 망설였는데, 이번엔 **블루스퀘어**라는 앱 덕분에 진짜 기분 좋게 다녀왔어요! 👗
    
    앱에서 **'{info['name']}'**을 검색하면 주변에서 가장 저렴하고 빈 공간이 있는 주차장을 리스트로 쫙 보여주더라고요. 최저가 정보부터 무료 주차권 유무까지 한눈에 확인 가능하니까, 이제 주차 걱정은 쓱- 지워버리고 우아하게 음식만 즐기면 된답니다! 
    
    [주차 스트레스 지우기: 블루스퀘어 앱 바로가기](https://bluesquare.example.com/install)
    """
