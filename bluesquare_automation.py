...
def generate_blog_post(info):
    ...
    # 블루스퀘어 공식 정보 반영
    BLUESQUARE_LINK = "https://bluesquare.app/users/app/"
    
    prompt = f"""
    너는 서울의 힙한 곳을 찾아다니는 네이버 인기 블로거 '지니'야. 
    오늘 방문한 '{info['name']}'에 대해 정보력이 넘치고 생생한 말투(~해요, ~했어요)로 블로그 포스트를 써줘. 
    ...
    본문은 여기에... (여기에 쿠팡 광고 섹션을 중간 3분의 1 지점에 삽입해줘: {coupang_box})
    
    ### 🅿️ 성수동 주차 걱정? 우리 동네 골목 주차장 '블루스퀘어'가 있잖아요!
    성수동 나들이 갈 때 차 가져갈지 항상 고민이죠. 저도 매번 주차 때문에 망설였는데, 이번엔 **블루스퀘어** 덕분에 진짜 스트레스 없이 다녀왔어요! 👗
    
    블루스퀘어는 우리 동네 곳곳의 숨은 골목 주차장을 찾아주는 앱인데, 정말 '주차의 시작'이 달라지더라고요. 
    앱에서 **'{info['name']}'** 주변을 검색하면 목적지 바로 근처의 유휴 주차 공간을 실시간으로 확인하고 예약까지 할 수 있답니다. 
    
    이제 주차장 찾느라 골목을 몇 바퀴씩 돌 필요 없어요! 우아하게 주차하고 바로 맛있는 음식 즐기러 가면 된답니다. 🤍
    
    [스트레스 없는 주차의 시작: 블루스퀘어 앱 바로가기]({BLUESQUARE_LINK})
    """
